...
import { X, Bot, Sparkles, AlertTriangle, TrendingUp, ArrowRight, ShieldCheck } from 'lucide-react';
...
export const GeneralManagerModal: React.FC = () => {
  ...
  const stalledTasks = gmSummary.stalledTasks || [];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl glass-panel p-6 sm:p-8 border border-purple-500/30 shadow-[0_0_50px_-10px_rgba(168,85,247,0.25)] text-right">
        ...
        <div className="flex items-center gap-3.5 mb-6">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-black text-white">تقرير الحالة الموثقة</h2>
            <p className="text-xs text-purple-300 font-medium mt-0.5">ملخص مبني على بيانات NAWAF HQ المسجلة فقط</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-950/40 via-indigo-950/30 to-slate-900/50 border border-purple-500/20 mb-6">
          <p className="text-xs sm:text-sm text-slate-200 leading-relaxed font-medium">{gmSummary.greeting}</p>
          <p className="text-xs text-slate-300 mt-2 leading-relaxed">{gmSummary.headline}</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="p-3 rounded-xl bg-slate-900/50 border border-white/5 text-center">
            ...
            <div className="text-[11px] text-slate-400 mt-0.5">موظفون بحالة عمل</div>
          </div>
          <div className="p-3 rounded-xl bg-slate-900/50 border border-white/5 text-center">
            <div className="text-xl font-bold text-emerald-400 font-mono">{metrics.completedTasks}</div>
            <div className="text-[11px] text-slate-400 mt-0.5">مهام مكتملة موثقة</div>
          </div>
          <div className="p-3 rounded-xl bg-slate-900/50 border border-white/5 text-center">
            <div className="text-xl font-bold text-cyan-400 font-mono">{metrics.productivityRate}%</div>
            <div className="text-[11px] text-slate-400 mt-0.5">اكتمال المهام المسجلة</div>
          </div>
        ...
        <div className="mb-6 space-y-2.5">
          <h3 className="text-xs font-bold text-slate-300 flex items-center gap-1.5 mb-2">
            ...
            <span>ملاحظات مشتقة من الحالة المسجلة:</span>
          </h3>
        ...
        {stalledTasks.length > 0 && (
          <div className="mb-6 p-4 rounded-2xl bg-amber-950/30 border border-amber-500/30">
            <h4 className="text-xs font-bold text-amber-300 mb-2 flex items-center gap-1.5">
              ...
              <span>مهام مسجلة كمتوقفة أو تحتاج تدخلاً:</span>
            </h4>
            <div className="space-y-1.5">
              {stalledTasks.map((task, idx) => (
        ...
        <div className="mb-6 p-4 rounded-2xl bg-cyan-950/30 border border-cyan-500/30">
          <div className="text-xs font-bold text-cyan-300 mb-1 flex items-center gap-1.5">
            ...
            <span>التركيز المقترح:</span>
          </div>
          <p className="text-xs text-slate-200 leading-relaxed">{gmSummary.recommendedFocus}</p>
        </div>

        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-white/5">
          <div className="text-[11px] text-slate-400 flex items-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span>لا يعرض هذا التقرير نجاحاً أو تكلفة أو إنجازاً غير مسجل في النظام.</span>
          </div>

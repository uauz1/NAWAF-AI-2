...
export const MainDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-right select-none">
      ...
      {/* 1. Executive Greeting & Status Line */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
        <div>
          ...
          <p className="text-xs sm:text-sm text-[var(--text-secondary)] mt-0.5">
            شركتك الذكية تعمل باستقلالية تامة وتلتزم بالقانون رقم 1 (التكلفة الصفرية $0.00).
          </p>
        </div>
      ...
      {/* 2. THE CHIEF OF STAFF EXECUTIVE BRIEFING LAYER */}
      <AdvisorBriefingPanel />

      {/* 3. THE CORE EXPERIENCE: Live 3D Headquarters */}
      <div className="w-full">
        <IsometricOffice3D />
      </div>

      {/* 4. Executive Contextual Operations Layer */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Card 1: Urgent CEO Approvals */}
        <div className="p-6 rounded-3xl bg-[var(--bg-card)] border border-[var(--border-subtle)] space-y-4 shadow-sm">
          ...
          {pendingDecisions.length === 0 ? (
            <div className="py-8 text-center text-[var(--text-muted)] text-xs flex flex-col items-center gap-2">
              ...
              <span>جميع القرارات التنفيذية معتمدة والفريق يعمل بسلاسة.</span>
            </div>
          ) : (
              ...
              {pendingDecisions.slice(0, 2).map((dec) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-xs font-bold text-[var(--text-primary)] leading-snug">
                      {dec.title}
                    </span>
        ...
        {/* Card 2: Core Strategic Projects */}
        <div className="p-6 rounded-3xl bg-[var(--bg-card)] border border-[var(--border-subtle)] space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            {projects.map((proj) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[var(--accent-primary)] font-mono font-bold">
                    {proj.progress}%
                  </span>
                </div>
        ...
        {/* Card 3: Live Autonomous Activity Stream */}
        <div className="p-6 rounded-3xl bg-[var(--bg-card)] border border-[var(--border-subtle)] space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-[var(--text-primary)]">نبض العمل المستقل</h2>
            </div>
            ...
            {recentActivities.map((act, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[var(--text-secondary)] text-[11px] mt-0.5 line-clamp-1 leading-snug">
                    {act.actionText}
                  </p>
                </div>
              </div>
